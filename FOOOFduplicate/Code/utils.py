...
def save_sim_data(file_name, folder, freqs, psds, sim_params):
    ...
    np.save(path_name + '_freqs.npy', freqs)
    np.save(path_name + '_psds.npy', psds)
    ## finish adding by AT
...
## This function is added by AT
def save_ground_truth(file_name, folder, peak_truths, ap_truths):
    """Save out the ground truth of the simulated data."""

    path_name = pjoin(DATA_PATH, folder, file_name)

    # Save as individual .npy files
    np.save(path_name + '_peak_truths.npy', peak_truths)
    np.save(path_name + '_ap_truths.npy', ap_truths)
## 
